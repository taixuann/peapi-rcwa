...
parser.add_argument('-t', type=float, required=True, help="Thickness of the under-air layer (t2)")  #Thickness of spacer under PEAPI
...
def S4Module(eV, e_r_PEAPI, e_i_PEAPI, n_PEAPI, d_PEAPI, D, h, k, N): #Defines a helper function to run S4 simulation for given photon energy eV and structure parameters.
    S = S4.New(Lattice=((1,0),(0,0)), NumBasis=N) #Lattice = ((D,0),(0,0)): 1D periodic structure with period D. NumBasis=N: number of Fourier modes used in RCWA.
    ...
    S.AddLayer('Above',0,'Air') #The simulation assumes light is coming from the top of the PEAI
    #S.AddLayer('ASpacer', (t2), 'Air')
    S.AddLayer('PEAPI', d_PEAPI, 'Perovskite')
    #S.AddLayer('BSpacer', (t2-n_PEAPI*d_PEAPI)*0.875,'Air')
    S.AddLayer('Below', 0,'Air') #This layer is semi-infinite air

    #---------------------------------------------------------------------------------------------------------%
    #Source construction
    #---------------------------------------------------------------------------------------------------------%
    #safe_arg = np.clip(k*1.2398419/eV, -1, 1)
    #angle = abs(np.arcsin(safe_arg) * 180 / np.pi)
...
while eV < (eV_max + eV_step/2): #Outer loop over photon energy
    ...
    while wv < (k_max + k_step/2):
        (Tc_0, Rc_0) = S4Module(eV, e_r_PEAPI, e_i_PEAPI, n_PEAPI, t1, d, h, wv, N)
